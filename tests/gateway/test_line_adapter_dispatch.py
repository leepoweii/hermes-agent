...
@pytest.mark.asyncio
@respx.mock
async def test_mention_only_message_silently_dropped(make_line_adapter):
    """A mention-only message (e.g. '@小茉' with no body) becomes empty after the
    strip pass and is silently dropped instead of dispatched to the LLM."""
    route = respx.post("https://api.line.me/v2/bot/message/reply").mock(
        return_value=Response(200, json={})
    )
    adapter = make_line_adapter(
    ...
    await adapter.dispatch_event(_group_msg_event(text="@小茉"))
    await adapter.wait_idle()
    assert not route.called
...
@pytest.mark.asyncio
@respx.mock
async def test_postback_without_reply_token_silently_returns(line_adapter_with_fast_llm):
    """Postback events lacking a replyToken should be logged and dropped, not crash."""
    route = respx.post("https://api.line.me/v2/bot/message/reply").mock(
        return_value=Response(200, json={})
    )
    event = {
        ...
        "source": {"type": "user", "userId": "U1"},
        "timestamp": 2,
        "postback": {"data": '{"action": "show_response", "request_id": "x"}'},
    }
    await line_adapter_with_fast_llm.dispatch_event(event)
    assert not route.called
...
@pytest.mark.asyncio
@respx.mock
async def test_dispatch_event_unhandled_event_type_drops_safely(line_adapter_with_fast_llm):
    """follow / join / leave / beacon events should silently complete without dispatching anything."""
    route = respx.post("https://api.line.me/v2/bot/message/reply").mock(
        return_value=Response(200, json={})
    )
    follow_event = {
    ...
    await line_adapter_with_fast_llm.dispatch_event(follow_event)
    await line_adapter_with_fast_llm.wait_idle()
    assert not route.called
