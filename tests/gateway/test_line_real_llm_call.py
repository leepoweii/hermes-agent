...
@pytest.mark.asyncio
async def test_real_llm_call_maps_room_source():
    """src_type='room' → chat_type='room', chat_id=roomId."""
    adapter = _make_adapter()
    captured: dict = {}
    ...
    adapter.set_message_handler(handler)
    out = await adapter._real_llm_call(
        "hello", {"type": "room", "userId": "U_member", "roomId": "R1"}
    )
    assert out == "ok"
    src = captured["event"].source
    assert src.chat_type == "room"
    assert src.chat_id == "R1"
    assert src.user_id == "U_member"
