...
@pytest.mark.asyncio
async def test_get_chat_info_maps_chat_id_prefix_to_type(line_adapter_with_fast_llm):
    """get_chat_info() should classify by ID prefix: C → group, R → room, U → dm."""
    info_user = await line_adapter_with_fast_llm.get_chat_info("U_user_id")
    info_group = await line_adapter_with_fast_llm.get_chat_info("C_group_id")
    info_room = await line_adapter_with_fast_llm.get_chat_info("R_room_id")
    assert info_user["type"] == "dm"
    assert info_group["type"] == "group"
    assert info_room["type"] == "room"
